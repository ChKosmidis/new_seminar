...
1) Убедитесь, что у вас установлена Node.js >= 18 и npm.
2) Откройте PowerShell и выполните из корня репозитория:
```powershell
npm run setup
npm run dev
```
это выполнит установку зависимостей внутри `app` и запустит dev-сервер Vite.
...
### Быстрый запуск и npm-скрипты
- Все команды npm выполняются из каталога `new_seminar`. Здесь лежит прокси-`package.json`, который перенаправляет вызовы во внутренний каталог `app`.
- Первая установка зависимостей: `npm run setup` (равнозначно `npm install --prefix app`).
- Запуск дев-сервера: `npm run dev` (внутри происходит `npm --prefix app run dev`).
- Production-сборка: `npm run build`.
- Проверка на незакрытые merge-конфликты: `npm run check:conflicts`. Этот шаг автоматически выполняется перед `dev` и `build`.
...
- [x] Реализовать страницу `1_community_building`, сохранив склонение текста, интерактивы (переписать их на TS). В процессе убедиться, что кириллица отображается без искажений.
- [x] Добавить `react-hook-form` или локальный стейт для форм, типизировать поля.
- [x] На базе этой структуры построить остальные модули (2…8, resources). Пока достаточно статической разметки + анимации.
- [x] Перенести на TypeScript страницу `2_internal_communications` с отдельными компонентами диагностики, инструментов и плана улучшений.
- [x] Привести страницу «Внутренние коммуникации» к общему стилю: единые отступы, корректная тёмная тема, понятные иконки раскрытия.
- [x] Привести страницу «Внутренние коммуникации» к общему стилю: единые отступы, корректная тёмная тема, понятные иконки раскрытия.
- [x] Переписать страницу `3_infrastructure` на TypeScript с интерактивным аудитом и планированием улучшений.
- [x] Привести страницу «Внутренние коммуникации» к общему стилю: единые отступы, корректная тёмная тема, понятные иконки раскрытия.
- [x] Переписать страницу `3_infrastructure` на TypeScript с интерактивным аудитом и планированием улучшений.
